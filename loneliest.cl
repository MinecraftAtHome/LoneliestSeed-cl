...
ulong rotl64(ulong x, uchar b)
{
    return (x << b) | (x >> (64-b));
}

  static inline ATTR(const, always_inline, artificial)
uint rotr32(uint a, uchar b)
{
    return (a >> b) | (a << (32-b));
}
...
///=============================================================================
  ...
  static inline void setSeed(ulong *seed, ulong value)
{
    *seed = (value ^ 0x5deece66d) & ((1ULL << 48) - 1);
}

  static inline int next(ulong *seed, const int bits)
{
    *seed = (*seed * 0x5deece66d + 0xb) & ((1ULL << 48) - 1);
    return (int) ((long)*seed >> (48 - bits));
}

  static inline int nextInt(ulong *seed, const int n)
{
    int bits, val;
    const int m = n - 1;

    if ((m & n) == 0) {
        ulong x = n * (ulong)next(seed, 31);
  ...
  static inline ulong nextLong(ulong *seed)
{
    return ((ulong) next(seed, 32) << 32) + next(seed, 32);
}

  static inline float nextFloat(ulong *seed)
{
    return next(seed, 24) / (float) (1 << 24);
}

  static inline double nextDouble(ulong *seed)
{
    ulong x = (ulong)next(seed, 26);
    x <<= 27;
    x += next(seed, 27);
    return (long) x / (double) (1ULL << 53);
}
 ...
 */
  static inline void skipNextN(ulong *seed, ulong n)
{
    ulong m = 1;
    ulong a = 0;
    ulong im = 0x5deece66dULL;
    ulong ia = 0xb;
    ulong k;
...
STRUCT(Xoroshiro)
{
  ...
  static inline void xSetSeed(Xoroshiro *xr, ulong value)
{
    const ulong XL = 0x9e3779b97f4a7c15ULL;
    const ulong XH = 0x6a09e667f3bcc909ULL;
    const ulong A = 0xbf58476d1ce4e5b9ULL;
    const ulong B = 0x94d049bb133111ebULL;
    ulong l = value ^ XH;
    ulong h = l + XL;
    l = (l ^ (l >> 30)) * A;
    h = (h ^ (h >> 30)) * A;
  ...
  static inline ulong xNextLong(Xoroshiro *xr)
{
    ulong l = xr->lo;
    ulong h = xr->hi;
    ulong n = rotl64(l + h, 17) + l;
    h ^= l;
  ...
  static inline int xNextInt(Xoroshiro *xr, uint n)
{
    ulong r = (xNextLong(xr) & 0xFFFFFFFF) * n;
    if ((uint)r < n)
    {
        while ((uint)r < (~n + 1) % n)
        {
  ...
  static inline void xSkipN(Xoroshiro *xr, int count)
{
  ...
  static inline ulong xNextLongJ(Xoroshiro *xr)
{
    int a = xNextLong(xr) >> 32;
    int b = xNextLong(xr) >> 32;
    return ((ulong)a << 32) + b;
}

  static inline int xNextIntJ(Xoroshiro *xr, uint n)
{
    int bits, val;
    const int m = n - 1;

    if ((m & n) == 0) {
        ulong x = n * (xNextLong(xr) >> 33);
        return (int) ((long) x >> 31);
    }
 ...
 */
  ...
  static inline ulong mcStepSeed(ulong s, ulong salt)
{
    return s * (s * 6364136223846793005ULL + 1442695040888963407ULL) + salt;
}

  static inline int mcFirstInt(ulong s, int mod)
{
    int ret = (int)(((long)s >> 24) % mod);
    if (ret < 0)
        ret += mod;
    return ret;
}

  static inline int mcFirstIsZero(ulong s, int mod)
{
    return (int)(((long)s >> 24) % mod) == 0;
}

  static inline ulong getChunkSeed(ulong ss, int x, int z)
{
    ulong cs = ss + x;
    cs = mcStepSeed(cs, z);
    cs = mcStepSeed(cs, x);
  ...
  static inline ulong getLayerSalt(ulong salt)
{
    ulong ls = mcStepSeed(salt, salt);
    ls = mcStepSeed(ls, salt);
    ls = mcStepSeed(ls, salt);
    return ls;
}

  static inline ulong getStartSalt(ulong ws, ulong ls)
{
    ulong st = ws;
    st = mcStepSeed(st, ls);
    st = mcStepSeed(st, ls);
  ...
  static inline ulong getStartSeed(ulong ws, ulong ls)
{
    ulong ss = ws;
...
STRUCT(Layer)
{
    ...
    char edge;        // maximum border required from parent layer
    int scale;          // scale of this layer (cell = scale x scale blocks)

    ulong layerSalt; // processed salt or initialization mode
    ulong startSalt; // (depends on world seed) used to step PRNG forward
    ulong startSeed; // (depends on world seed) start for chunk seeds
    ...
    SplineStack *ss, float f, float g, float h, float i, float j, float k, int bl)
{
  ...
  ATTR(hot, const)
static inline double indexedLerp(uchar idx, double a, double b, double c)
{
   switch (idx & 0xf)
   {
   case 0:  return  a + b;
...
#else
    uchar a1 = idx[h1]   + h2;
    uchar b1 = idx[h1+1] + h2;

    uchar a2 = idx[a1]   + h3;
    uchar b2 = idx[b1]   + h3;
    uchar a3 = idx[a1+1] + h3;
    uchar b3 = idx[b1+1] + h3;
  ...
  void initBiomeNoise(BiomeNoise *bn, int mc)
{
  ...
  double sampleClimatePara(const BiomeNoise *bn, long *np, double x, double z)
{
  ...
  ulong get_np_dist(__generic const ulong np[6], const BiomeTree *bt, int idx)
{
  ...
  int get_resulting_node_new(__generic const ulong np[6], const BiomeTree *bt) {
    ...
    for (size_t i = 0; i < 9112; i++) {
        ...
        // best_distance = biome;
        ulong dist = get_np_dist(np, bt, i);
        // (void)dist;
...
static const int btree20_param[][2] =
{
 ...
 static const ulong btree20_nodes[] =
{
    ...
    //   |               are a char index to the first child node.
    //   |
...
typedef struct {
  ...
  StackFrame new_frame(int index, int depth) {
    StackFrame frame;
    frame.depth = depth;
    frame.index = index;
    return frame;
  ...
  int get_resulting_node_improved(__generic const ulong np[6], const BiomeTree *bt) {
    ...
    ulong best_dist = 18446744073709551615ULL;
    int best_index = -1;
    int visited_nodes = 0;
    ...
    while (stack.pointer > 0) {
        ...
        ulong node = btree20_nodes[frame.index];
        visited_nodes++;

        // if the node is a leaf
        if (((node >> 8 * 7) & 0xFF) == 0xFF) {
            ulong dist = get_np_dist(np, bt, frame.index);
 ...
 static const BiomeTree g_btree[MC_NEWEST - MC_1_18 + 1] =
{
    // MC_1_20
...
/// Biome sampler for MC 1.18
  int sampleBiomeNoise(const BiomeNoise *bn, long *np, int x, int y, int z,
    ulong *dat, uint sample_flags)
{
    if (bn->nptype >= 0)
    ...
    long l_np[6];
    long *p_np = np ? np : l_np;
    p_np[0] = (long)(10000.0F*t);
    p_np[1] = (long)(10000.0F*h);
    p_np[2] = (long)(10000.0F*c);
    p_np[3] = (long)(10000.0F*e);
    p_np[4] = (long)(10000.0F*d);
    p_np[5] = (long)(10000.0F*w);

    int id = none;
    if (!(sample_flags & SAMPLE_NO_BIOME))
        id = climateToBiome(bn->mc, (const ulong*)p_np, dat);
    return id;
}

  void setupGenerator(Generator *g, int mc, uint flags) {
    g->mc = mc;
    g->dim = DIM_UNDEF;
    g->flags = flags;
    g->seed = 0;
...
{
    static const ulong md5_octave_n[][2] = {
    ...
    ulong xlo = xNextLong(xr);
    ulong xhi = xNextLong(xr);
    int i = 0, n = 0;
        ...
        PerlinNoise *octaves, const double *amplitudes, int omin, int len, int nmax)
{
  ...
  static int init_climate_seed(
    ...
    ulong xlo, ulong xhi, int large, int nptype, int nmax
    )
{
    Xoroshiro pxr;
    int n = 0;
  ...
  void setBiomeSeed(BiomeNoise *bn, ulong seed, int large)
{
    Xoroshiro pxr;
    xSetSeed(&pxr, seed);
    ulong xlo = xNextLong(&pxr);
    ulong xhi = xNextLong(&pxr);
  ...
  void applySeed(Generator *g, int dim, ulong seed)
{
    g->dim = dim;
    g->seed = seed;
    g->sha = 0;
...
STRUCT(StructureConfig)
{
  ...
  static inline ATTR(const)
Pos getFeatureChunkInRegion(StructureConfig config, ulong seed, int regX, int regZ)
{
    /*
    ...
    */
    Pos pos;
    const ulong K = 0x5deece66dULL;
    const ulong M = (1ULL << 48) - 1;
    const ulong b = 0xb;
  ...
  static inline ATTR(const)
Pos getFeaturePos(StructureConfig config, ulong seed, int regX, int regZ)
{
    Pos pos = getFeatureChunkInRegion(config, seed, regX, regZ);

    pos.x = (int)(((ulong)regX*config.regionSize + pos.x) << 4);
    pos.z = (int)(((ulong)regZ*config.regionSize + pos.z) << 4);
    return pos;
}
    ...
    const Layer *layer, int areaX, int areaZ, int *maxX, int *maxZ, size_t *siz)
{
  ...
  static inline void getVoronoiCell(ulong sha, int a, int b, int c,
        int *x, int *y, int *z)
{
    ulong s = sha;
    s = mcStepSeed(s, a);
  ...
  void voronoiAccess3D(ulong sha, int x, int y, int z, int *x4, int *y4, int *z4)
{
    x -= 2;
    y -= 2;
    z -= 2;
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        ulong d = rx*(ulong)rx + ry*(ulong)ry + rz*(ulong)rz;
        if (d < dmin)
        {
            dmin = d;
            ax = cx;
...
{
    ulong dat = 0;
    ulong *p_dat = opt ? &dat : NULL;
    uint flags = opt ? SAMPLE_NO_SHIFT : 0;
    int i, j, k;
    int *p = out;
  ...
  Range getVoronoiSrcRange(Range r)
{
  ...
  int genBiomeNoiseScaled(const BiomeNoise *bn, int *out, Range r, ulong sha)
{
    if (r.sy == 0)
        r.sy = 1;

    ulong siz = (ulong)r.sx*r.sy*r.sz;
    int i, j, k;
            ...
            for (j = 0; j < r.sz; j++)
            {
                for (i = 0; i < r.sx; i++)
                {
                    ...
                    if (src)
                    {
                        ...
                        *p = src[(long)y4*s.sx*s.sz + (long)z4*s.sx + x4];
                    }
                    else
                    {
  ...
  size_t getMinLayerCacheSize(const Layer *layer, int sizeX, int sizeZ)
{
  ...
  inline static
ulong chunkGenerateRnd(ulong worldSeed, int chunkX, int chunkZ)
{
    ulong rnd;
  ...
  int getBiomeAt(const Generator *g, int scale, int x, int y, int z)
{
  ...
  int getVariant(StructureVariant *r, int structType, int mc, ulong seed,
        int x, int z, int biomeID)
{
  ...
  int isViableStructurePos(int structureType, Generator *g, int x, int z, uint flags)
{
